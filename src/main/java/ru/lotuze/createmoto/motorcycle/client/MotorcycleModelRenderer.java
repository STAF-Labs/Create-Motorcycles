...
public class MotorcycleModelRenderer {
    ...
    private static final float FOCUS_OFFSET = 0.06F;
    private static final float FOCUS_BRIGHTNESS = 1.35F;

    private List<ModelElement> elements;
    private List<ModuleBounds> moduleBounds;

    public List<ModuleBounds> getModuleBounds() {
        if (this.moduleBounds == null) {
            this.moduleBounds = buildModuleBounds();
        }

        return this.moduleBounds;
    }
    ...
    public void render(
        ...
        for (ModelElement element : getElements()) {
            renderElement(element, poseStack, consumer, packedLight, steeringAngle, wheelRotation, null);
        }
    }
    ...
    private static List<ModelElement> loadElements() {
        ...
        for (ModuleSpec module : MODULES) {
            ResourceLocation location = ResourceLocation.fromNamespaceAndPath(
                    ...
                    "models/entity/motorcycle/modules/" + module.name + ".json"
            );
                    ...
                    .ifPresent(resource -> {
                        try (
                            ...
                            for (var jsonElement : root.getAsJsonArray("elements")) {
                                loaded.add(ModelElement.fromJson(
                            ...
                            CreateMotorcycles.LOGGER.warn(
                                    "Failed to load motorcycle model module {}",
                                    location,
                                    exception
                            );
                        }
    ...
    private void renderElement(
        ...
        boolean focused = element.module().equals(focusedModule);

        applyElementTransform(poseStack, element, steeringAngle, wheelRotation, focusedModule);
        ...
        int color = colorFor(element.color);

        if (focused) {
            color = brighten(color, FOCUS_BRIGHTNESS);
        }

        cube(poseStack.last(), consumer, x1, y1, z1, x2, y2, z2, color, packedLight);
    ...
    private void applyElementTransform(
        ...
        if (element.module().equals(focusedModule)) {
            applyFocusOffset(poseStack, element.module());
        }
        ...
        if ("moto_front_wheel".equals(element.module())) {
            rotateWheel(poseStack, FRONT_WHEEL_PIVOT_X, FRONT_WHEEL_PIVOT_Y, FRONT_WHEEL_PIVOT_Z, wheelRotation);
        } else if ("rear_wheel".equals(element.module())) {
            rotateWheel(poseStack, REAR_WHEEL_PIVOT_X, REAR_WHEEL_PIVOT_Y, REAR_WHEEL_PIVOT_Z, wheelRotation);
        }

        if (element.rotation != null) {
            poseStack.translate(
                    toWorld(element.rotation.origin[0]),
                    toWorld(element.rotation.origin[1]),
                    toWorld(element.rotation.origin[2])
            );
    ...
    public Matrix4f getPickTransform(
            int elementIndex,
        ...
        ModelElement element = elements.get(elementIndex);

        PoseStack poseStack = new PoseStack();

        applyElementTransform(poseStack, element, motorcycle.getSteeringAngle(partialTick), motorcycle.getWheelRotation(partialTick), focusedModule);
    ...
    private record ModelRotation(
            String axis,
            float angle,
            float[] origin
    ) {
        static ModelRotation fromJson(
                JsonObject object,
                float[] offset
        ) {
            ...
            return new ModelRotation(
                    ...
                    new float[]{
                            origin.get(0).getAsFloat() + offset[0],
                            origin.get(1).getAsFloat() + offset[1],
                            origin.get(2).getAsFloat() + offset[2]
                    }
            );
        }
    }
    ...
    private record ModelElement(
        ...
        static ModelElement fromJson(
            ...
            ModelRotation rotation = object.has("rotation")
                    ? ModelRotation.fromJson(object.getAsJsonObject("rotation"), offset)
                    : null;

            int color = object.has("color")
                    ? object.get("color").getAsInt()
                    : 0;

            return new ModelElement(module,
                    new float[]{
    ...
    private List<ModuleBounds> buildModuleBounds() {
        List<ModuleBounds> bounds = new ArrayList<>();
        ...
        for (ModuleSpec module : MODULES) {
            ...
            for (ModelElement element : getElements()) {
                if (!module.name().equals(element.module())) {
                    continue;
                }

                minX = Math.min(minX, Math.min(element.from[0], element.to[0]));
                minY = Math.min(minY, Math.min(element.from[1], element.to[1]));
                minZ = Math.min(minZ, Math.min(element.from[2], element.to[2]));

                maxX = Math.max(maxX, Math.max(element.from[0], element.to[0]));
                maxY = Math.max(maxY, Math.max(element.from[1], element.to[1]));
                maxZ = Math.max(maxZ, Math.max(element.from[2], element.to[2]));
    ...
    private void applyFocusOffset(PoseStack poseStack, String module) {
        ModuleBounds bounds = getModuleBounds().stream()
                .filter(value -> value.module().equals(module))
                .findFirst()
                .orElse(null);
        ...
        poseStack.translate(
                centerX / length * FOCUS_OFFSET,
                centerY / length * FOCUS_OFFSET,
                centerZ / length * FOCUS_OFFSET
        );
    }
    ...
    private static int brighten(int color, float factor) {
        ...
        red = Math.min(255, Math.round(red * factor));
        green = Math.min(255, Math.round(green * factor));
        blue = Math.min(255, Math.round(blue * factor));

        return alpha << 24 | red << 16 | green << 8 | blue;
    }

    public List<PickElement> getPickElements() {
        List<ModelElement> elements = getElements();
        List<PickElement> pickElements = new ArrayList<>(elements.size());

        for (int i = 0; i < elements.size(); i++) {
            ModelElement element = elements.get(i);

            pickElements.add(
                    new PickElement(i, element.module(),
                            toWorld(Math.min(element.from[0], element.to[0])),
                            toWorld(Math.min(element.from[1], element.to[1])),
                            toWorld(Math.min(element.from[2], element.to[2])),
                            toWorld(Math.max(element.from[0], element.to[0])),
                            toWorld(Math.max(element.from[1], element.to[1])),
                            toWorld(Math.max(element.from[2], element.to[2]))
                    )
            );
        }
